...
contract CSVOracle is AccessControl, ICSVOracle {
    ...
    function _checkAndFinalizeValuation(bytes32 submissionId) internal {
        ...
        // Count unique attestors for this submission
        address[] memory uniqueAttestors = new address[](subs.length);
        uint256 uniqueCount = 0;
        
        for (uint256 i = 0; i < subs.length; i++) {
            bool isUnique = true;
            for (uint256 j = 0; j < uniqueCount; j++) {
                if (uniqueAttestors[j] == address(0)) break; // This shouldn't happen but safety check
            }
            if (isUnique) {
                uniqueAttestors[uniqueCount] = address(0); // Placeholder, we need to track attestor addresses properly
                uniqueCount++;
            }
        }
    ...
    /**
     * @dev Get list of all trusted attestors
     */
    function getTrustedAttestors() external view returns (address[] memory) {
        // This is a simplified version - in practice you'd maintain an array
        uint256 count = 0;
        address[] memory attestors = new address[](10); // Max 10 for gas efficiency
        
        // In a real implementation, you'd iterate through a stored array
        // For now, returning empty array as example
        assembly {
            mstore(attestors, count)
        }
