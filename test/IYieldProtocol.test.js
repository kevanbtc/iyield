...
describe("iYield Protocol Core Tests", function () {
    ...
    describe("CSV Oracle Functionality", function () {
        ...
        it("Should detect stale attestations", async function () {
            ...
            // Should not be stale with reasonable max age
            expect(await csvOracle.isStale(samplePolicyId, 3600)).to.be.false;
        });
    });
    ...
    describe("iYield Token Functionality", function () {
        beforeEach(async function () {
            // Setup attestation
    ...
    describe("Integration Tests", function () {
        it("Should enforce compliance on token transfers", async function () {
            // Setup everything for token issuance
            const csvValue = ethers.parseEther("100000");
            const tokensToIssue = ethers.parseEther("80000");
            ...
            // Setup compliance for investor1 only (not investor2)
            const profile = {
                investorType: 1, // ACCREDITED
                kycTimestamp: Math.floor(Date.now() / 1000),
                accreditationExpiry: Math.floor(Date.now() / 1000) + 365 * 24 * 60 * 60,
            ...
            // Try to transfer to non-compliant investor2 - should fail
            await expect(
                iYieldToken.connect(investor1).transfer(investor2.address, ethers.parseEther("1000"))
            ).to.be.revertedWithCustomError || await expect(
                iYieldToken.connect(investor1).transfer(investor2.address, ethers.parseEther("1000"))
            ).to.be.reverted;
        });
    });
